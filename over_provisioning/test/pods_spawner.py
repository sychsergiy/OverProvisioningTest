...
class PodsSpawner:
    ...
    def create_pod(
        self, pod_name_suffix: str, max_pod_creation_time: float
    ) -> t.Tuple[str, float]:
        """
        ...
        pod_creation_time = self._pod_creator.create_pod(
            pod_name, self._pod_spec
        )
        ...
        (
            time_limit_not_hited,
            waited_time,
        ) = self._pod_waiter.wait_on_running_status(
